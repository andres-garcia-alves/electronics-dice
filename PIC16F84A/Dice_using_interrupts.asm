...
setup
	...
	bcf		OPTION_REG, NOT_RBPU ; pull-ups internos activados
	bcf		OPTION_REG, INTEDG	; INT por flanco de bajada

	bcf		STATUS, RP0			; volver al banco 0 de memoria

	movlw	b'10010000'
	movwf	INTCON				; habilitar flags de interrup. externas + interrup. globales
...
; Manejo de interrupción (boton presionado)
interruption
	call	delay_30ms			; rutina anti-rebote, 30 ms
	btfsc	PORTB, PIN_0		; ¿sigue en estado bajo el pin 0?
	call	exit_int			; no -> falso positivo, fin de interrupción
...
randomizer_loop
	...
	btfss	PORTB, PIN_0		; ¿sigue en estado bajo el pin 0?
...
exit_int
	...
	bcf		INTCON, INTF		; descartar la interrupción externa
	retfie						; habilitar flag de interrupciones globales
